...
void Setup(){
    ...
    GLuint leftWins = LoadTexture("winner_left.jpg");
    GLuint rightWins = LoadTexture("winner_right.jpg");
    ...
    glMatrixMode(GL_PROJECTION); glOrtho(-1.33, 1.33, -1.0, 1.0, -1.0, 1.0);
...
bool ProcessEvents(){
    ...
    while (SDL_PollEvent(&event)) {
        ...
        paddle1->y = (((float)(600 - event.motion.y) / 600.0f) * 2.0f) - 1.0f;
        }
    }
    return true;
}
...
void Update(){
    ...
    else if (paddle2->y < -0.8f){
        paddle2->y = -0.8f;
    }
    
    if (ball->y>0.95f&&ball->direction_y > 0.0f){
        ball->direction_y = -(ball->direction_y);
    }
    
    else if (ball->y<-0.95f&&ball->direction_y < 0.0f){
        ball->direction_y = -(ball->direction_y);
    }
        
    if (ball->x < -1.1f && ball->x > -1.3 && ball->y > paddle2->y - 0.25f && ball->y < paddle2->y + 0.25f){
        leftWon->Draw();
        ball->direction_x = fabs(ball->direction_x);
    }
    else if (ball->x > 1.1f && ball->x < 1.3 && ball->y > paddle1->y - 0.25f && ball->y < paddle1->y + 0.25f){
        leftWon->Draw();
        ball->direction_x = -(fabs(ball->direction_x));
    }
}
